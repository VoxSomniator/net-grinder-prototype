...
func check_config():
	...
	if err == OK: # if not, something went wrong with the file loading
		...
		# Controls: General
		if not user_config.has_section_key("controls:general", "show_scoreboard"):
			user_config.set_value("controls:general", "show_scoreboard", ["show_scoreboard", "Key", "Tab"])
		if not user_config.has_section_key("controls:general", "screenshot"):
			user_config.set_value("controls:general", "screenshot", ["screenshot", "Key", "F11"])

		# Controls: Mech / Misc
		if not user_config.has_section_key("controls:mech/misc", "freelook"):
			user_config.set_value("controls:mech/misc", "freelook", ["freelook", "Key", "K", "Key", "J"])
...
#	err = user_config.load("user://user.ini")
	...
	if err == OK: # if not, something went wrong with the file loading
		...
		user_config.set_value("hud", "secondary_color", hud_secondary_color)
		user_config.set_value("hud", "tertiary_color", hud_tertiary_color)
		user_config.save("user://user.ini")
